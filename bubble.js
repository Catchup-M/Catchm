// bubble.js
const BubbleMessage = ({ msg, swipedMessage, setSwipedMessage, index, handleTouchStart, handleTouchMove, handleTouchEnd }) => {
  // Message length checks
  const isLong = msg.text && msg.text.length > 50;
  const isMedium = msg.text && msg.text.length > 30 && msg.text.length <= 50;
  const isShort = msg.text && msg.text.length <= 30;

  const showDateBadge = index === 0 || (index > 0 && msg.date !== undefined && msg.date !== null && msg.date !== msg.prevDate);
    ...
    <React.Fragment key={index}>
      ...
      >
        ...
        {msg.type === 'image' ? (
          ...
          // Text messages (long/medium/short use same structure with slight differences)
          <>
            {(isLong || isMedium || isShort) && (
              <div style={{
                background: msg.sent ? '#dcf8c6' : '#fff',
                borderRadius: '12px',
                padding: isLong ? '8px 10px 4px 10px' : '8px 10px',
                maxWidth: '85%',
                maxWidth: isLong ? '290px' : '85%',
                boxShadow: '0 1px 2px rgba(0,0,0,0.1)',
                ...
                <div style={{
                  ...
                  <span style={{ whiteSpace: msg.replyTo ? 'normal' : 'nowrap' }}>{msg.text}</span>
                  <span style={{
                    fontSize: '11px',
                    color: '#6b7280',
                    display: 'flex',
